...
import { ChartData, TimeSeriesData } from '../lib/types';
...
export const DataVisualization: React.FC<DataVisualizationProps> = ({ 
  ...
  const isBarChart = data.datasets?.[0] && 'backgroundColor' in data.datasets[0] && Array.isArray(data.datasets[0].backgroundColor);
  
  let chartJsData;
  if (isBarChart) {
    chartJsData = {
      labels: data.labels,
      datasets: (data.datasets as ChartData['datasets']).map(ds => ({
        label: ds.label,
        data: ds.data,
    ...
    chartJsData = {
      labels: data.labels,
      datasets: (data.datasets as TimeSeriesData['datasets']).map(ds => ({
        label: ds.label,
        data: ds.data,
        backgroundColor: ds.backgroundColor,
        borderColor: ds.borderColor,
